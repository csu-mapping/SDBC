function p_value = flowCSRtests(flows, IDX, mc_times, R)
...
IX = unique(IDX(IDX>0));
cluster_num = length(IX);
if cluster_num>0
    clust = struct('Label', cell(cluster_num,1),...
        'Members', cell(cluster_num,1),...
        'Density', cell(cluster_num,1),...
        'OBox', cell(cluster_num,1),...
        'DBox', cell(cluster_num,1),...
        'Pvalue',cell(cluster_num,1));
    
    for i=1:cluster_num
        clust(i).Label = IX(i);
        clust(i).Members = find(IDX==clust(i).Label);
        clust(i).Density = length(clust(i).Members);
        
        x = flows(clust(i).Members, 1:2);
        k = convhull(x(:,1), x(:,2));
        x = x(k,:);
        warning off;
        [bx, by] = bufferm2('xy', x(:,1), x(:,2), R, 'out');
        clust(i).OBox = [bx, by];
        
        x = flows(clust(i).Members, 3:4);
        k = convhull(x(:,1), x(:,2));
        x = x(k,:);
        [bx, by] = bufferm2('xy', x(:,1), x(:,2), R, 'out');
        clust(i).DBox = [bx, by];
    ...
    npts = size(flows, 1);
    mc_density = zeros(cluster_num, mc_times);
    %fs = [flows(:,1:2); flows(:,3:4)];
    for r=1:mc_times
        %rand_flows = fs(randperm(2*npts), 1:2);
        %rand_flows = [rand_flows(1:npts, 1:2), rand_flows((npts+1):2*npts, 1:2)];
        rand_flows = [flows(randperm(npts),1:2), flows(randperm(npts),3:4)];
        for j=1:cluster_num
            isInOBox = inpolygon(rand_flows(:,1), rand_flows(:,2), clust(j).OBox(:,1), clust(j).OBox(:,2));
            isInDBox = inpolygon(rand_flows(:,3), rand_flows(:,4), clust(j).DBox(:,1), clust(j).DBox(:,2));
            mc_density(j, r) = sum(isInOBox & isInDBox);
        end
    end
    ...
    p_value = ones(npts, 1);
    for i=1:cluster_num
        clust(i).Pvalue = sum(mc_density(i,:)>=clust(i).Density)/mc_times;
...
%csu_bufferm  Computes buffer zone around a polygon.
%
% Usage:
%   [latb,lonb] = csu_bufferm(lat,lon,dist,direction)
%   [latb,lonb] = csu_bufferm(lat,lon,dist,direction,npts)
%   [latb,lonb] = csu_bufferm(lat,lon,dist,direction,npts,outputformat)
%   [xb,  yb]   = csu_bufferm('xy',x,y,dist,direction,npts,outputformat)
%
